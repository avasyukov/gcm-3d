GCM_Tetr_Plastic_Interpolation_1stOrder::GCM_Tetr_Plastic_Interpolation_1stOrder()
{
	num_method_type.assign("1st order interpolation on tetr mesh (with plasticity)");
...
int GCM_Tetr_Plastic_Interpolation_1stOrder::prepare_part_step(ElasticNode* cur_node, int stage)
{
...
void GCM_Tetr_Plastic_Interpolation_1stOrder::drop_deviator(ElasticNode* cur_node, ElasticNode* new_node)
{
...
int GCM_Tetr_Plastic_Interpolation_1stOrder::do_next_part_step(ElasticNode* cur_node, ElasticNode* new_node, float time_step, int stage, TetrMesh* mesh)
{
	if(stage == 3) {
...
//				previous_nodes[count].coords[1], previous_nodes[count].coords[2], cur_node);
			if( tmp_tetr != NULL )
			{
				...
				{
					if(logger != NULL)
						logger->write(string("Error: GCM_Tetr_Plastic_Interpolation_1stOrder::do_next_part_step - interpolation failed!"));
					return -1;
				}
				inner[i] = true;
			} else {
...
int GCM_Tetr_Plastic_Interpolation_1stOrder::get_number_of_stages()
{
	return 4;
};
