...
{
	if(stage == 0) {
		if(elastic_matrix3d[stage]->prepare_matrix(cur_node->la,cur_node->mu,cur_node->rho,1,0,0,logger) < 0)
			return -1;
	} else if(stage == 1) {
		if(elastic_matrix3d[stage]->prepare_matrix(cur_node->la,cur_node->mu,cur_node->rho,0,1,0,logger) < 0)
			return -1;
	} else if(stage == 2) {
		if(elastic_matrix3d[stage]->prepare_matrix(cur_node->la,cur_node->mu,cur_node->rho,0,0,1,logger) < 0)
			return -1;
	} else {
		return -1;
	}
...
int GCM_Tetr_Plastic_Interpolation_1stOrder_Advanced::do_next_part_step(ElasticNode* cur_node, ElasticNode* new_node, float time_step, int stage, TetrMesh* mesh)
{
	...
	else
	{
		...
		{
			stringstream ss;
			ss << "Error: GCM_Tetr_Plastic_Interpolation_1stOrder_Advanced::do_next_part_step - 'outer' values do not match. There are " << outer_count << " of them.";
			logger->write(ss.str());
		}
		return -1;
	}
...
float GCM_Tetr_Plastic_Interpolation_1stOrder_Advanced::get_max_lambda(ElasticNode* node)
{
