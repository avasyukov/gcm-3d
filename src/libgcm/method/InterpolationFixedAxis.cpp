													...
													float time_step, int stage, Mesh* genericMesh)
{
	...
	int outer_count = mesh->prepare_node(cur_node, &elastic_matrix3d,
						time_step, stage,
	...
	if( cur_node->isBorder() )
	{
		...
		else if ( outer_count == 3 )
		{
			...
			else
			{
				...
				// FIXME - WA
				Mesh* tetrMesh = (Mesh*) engine->getBody(virt_node->contactNodeNum)->getMeshes();
				int virt_outer_count = tetrMesh->prepare_node( virt_node, &virt_elastic_matrix3d,
						time_step, stage,
