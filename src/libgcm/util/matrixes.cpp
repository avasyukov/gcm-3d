...
bool gcm::gcm_matrix::operator|=(const gcm_matrix &A) const {
	...
	for (int i = 0; i < GCM_MATRIX_SIZE; ++i)
		for (int j = 0; j < GCM_MATRIX_SIZE; ++j) {
			...
			if ( fabs(p[i][j]) > EQT && fabs(A.p[i][j]) >  EQT) {
				if (fabs(p[i][j] - A.p[i][j]) > 0.01 * norm) {
					LOG_ERROR((*this - A)/A << "i, j = " << i << ", " << j << 
							"\t err = " << fabs(p[i][j] - A.p[i][j])/norm);
					return false;
				}
			} else if ((fabs(p[i][j]) < EQT && fabs(A.p[i][j]) >  EQT) || (fabs(p[i][j]) > EQT && fabs(A.p[i][j]) <  EQT)) 
				return false;
		}
	return true;
};
